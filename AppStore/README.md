...
**Izzy** is the production scheme and has no simulated StoreKit configuration. **IzzyStore** is for local StoreKit testing, with the Japanese ¥900 non-consumable. The `.storekit` file is copied into test bundles only. Debug UI-test paid access is removed by compilation from Release; archive validation checks for launch-argument leaks. Normal Debug launches use real StoreKit too.
