...
@MainActor final class IzzyUITests: XCTestCase {
    ...
    func testIdiomsBrowseSearchDetailsSpeakingAndSavedPersistence() {
        ...
        XCTAssertTrue(app.staticTexts["1,055 idioms"].waitForExistence(timeout: 3))
    ...
    func testNativePurchaseReviewScreenshots() throws {
        let config = try XCTUnwrap(Bundle(for: Self.self).url(forResource: "Izzy", withExtension: "storekit"))
    ...
    func testCompleteCollectionNewEntriesAndAliases() {
        ...
        app.tabBars.buttons["Phrases"].tap()
        groupByVerb()
        XCTAssertTrue(app.staticTexts["559 verbs"].waitForExistence(timeout: 3))
        app.searchFields.firstMatch.tap()
