...
@MainActor final class MemoryAndAccessUITests: XCTestCase {
    ...
    func testDesignDarkReducedMotionAndLargeReview() {
        ...
        XCTAssertTrue(app.staticTexts["1,055 idioms"].exists)
    ...
    func testFreeCatalogUnlockAndRefundHidePaidContent() throws {
        let url = try XCTUnwrap(Bundle(for: Self.self).url(forResource: "Izzy", withExtension: "storekit"))
        ...
        app.tabBars.buttons["Phrases"].tap()
        XCTAssertTrue(app.staticTexts["2,206 phrases"].waitForExistence(timeout: 5))
        ...
        XCTAssertTrue(app.staticTexts["Izzy Pro"].waitForExistence(timeout: 3))
    ...
    func testFreeSearchAndProCatalog() {
        launch(["--free-access"])
        app.tabBars.buttons["Phrases"].tap()
        XCTAssertTrue(app.staticTexts["2,206 phrases"].waitForExistence(timeout: 3))
