...
The collection contains **3020 expressions**: 80 original meaning-specific lessons, 534 additions from the supplied 601-row CSV, 848 additional phrasal-verb lessons, and 1558 idiom lessons with two authored contexts each. The original lessons retain their 160 situations, IDs, explanations, and learning history. The 67 overlapping CSV rows appear under **More usage** in their existing phrase details. All CSV Japanese meanings, English meanings, and examples are retained; combined labels are normalized with searchable aliases. The 848 additional lessons cover everyday life, travel, digital tasks and work, with bilingual meanings, usage notes, editorial difficulty and dictionary references. See [the latest expansion record](docs/CATALOG-3020.md).
...
**Home shows each phrase's American IPA** under the phrase (bring up → /brɪŋ ʌp/, get across → /ɡet əˈkrɔːs/). `scripts/build_pronunciations.py` writes `scripts/data/pronunciations.json` from the CMU Pronouncing Dictionary ([licence](scripts/data/cmudict-LICENSE.txt), shown in **Settings → Licenses**) and `scripts/data/pronunciation-overrides.json`, which holds missing words and the reading for words such as close, live and wind in each phrase. Stress is marked only inside words of two or more syllables, and function words take their weak form except at the end of a phrase (look forward to → /lʊk ˈfɔːrwɚd tuː/). After adding lessons, run `scripts/create_catalog.py`, then `scripts/build_pronunciations.py`, then `scripts/create_catalog.py` again; `scripts/test_collection.py` checks that every lesson has one.
...
Izzy is prepared as a free download with one non-consumable Izzy Pro purchase (intended Japan price ¥900). Speaking and all five story scenes are free. Free catalog access includes 50 fixed phrasal verbs and 50 fixed idioms for browsing, spaced reviews and speaking practice, plus all core images. Pro unlocks the full 3020-expression catalog for browsing and meaning reviews. StoreKit provides the displayed price, verified entitlements, restoration and transaction updates.
