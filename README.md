...
**Settings → iCloud** keeps learning history, saved phrases and notes in the learner's own private iCloud database, so they follow an Apple Account to a new device and stay the same on an iPhone and an iPad. It is on unless it is turned off there, and there is no account to make. Two devices that both changed something offline are merged rather than overwritten: reviews per phrase, practice events by ID, saved phrases as a set, and both notes where both were written. See [learning data in iCloud](docs/icloud-sync.md) for the record, the merge rules and what is still unverified.
