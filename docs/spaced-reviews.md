...
Izzy Pro is the non-consumable `me.unvalley.izzy.pro.lifetime`. The rename moved the product ID off `verve`, so no earlier purchase unlocks it; see [RENAME.md](RENAME.md). There is no subscription. StoreKit remains the source of entitlement and localized pricing.
