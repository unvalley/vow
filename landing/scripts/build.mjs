...
<section class="plans section" id="plans" aria-labelledby="plans-title"><div class="section-heading"><h2 id="plans-title">${t.plansTitle}</h2></div><div class="plan-grid"><article class="plan"><h3>${t.free}</h3><p class="plan-amount">${t.freeTag}</p><ul>${t.freeItems.map(s=>`<li>${check}${s}</li>`).join('')}</ul></article><article class="plan pro"><h3>Izzy Pro</h3><p class="plan-amount">${t.proTag}</p><p class="plan-price-note">${t.proPriceNote}</p><ul>${t.proItems.map(s=>`<li>${check}${s}</li>`).join('')}</ul></article></div><p class="plan-note">${t.planNote}</p></section>
