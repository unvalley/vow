...
const pauseIcon = '<svg class="icon-pause" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><rect x="7" y="6" width="3.5" height="12" rx="1"/><rect x="13.5" y="6" width="3.5" height="12" rx="1"/></svg>';
const playIcon = '<svg class="icon-play" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8.5 6.3v11.4c0 .8.9 1.3 1.5.9l8.6-5.7c.6-.4.6-1.3 0-1.7L10 5.4c-.6-.4-1.5 0-1.5.9Z"/></svg>';
...
// The capture is the bare screen, rounded like the display; no device is drawn around it.
const screenImage = (name, alt) => `<img class="screen" src="/assets/${name}-660.webp" srcset="/assets/${name}-440.webp 440w, /assets/${name}-660.webp 660w, /assets/${name}-990.webp 990w" sizes="(max-width: 900px) 260px, 278px" width="660" height="1434" alt="${esc(alt)}" loading="lazy" decoding="async">`;
// The app icon sits to the left of the wordmark wherever the logo appears.
const logo = '<img class="brand-icon" src="/assets/favicon.png" width="192" height="192" alt=""><img class="brand-wordmark" src="/assets/wordmark.svg" alt="Izzy" width="106" height="60">';
...
function page(lang) {
 const screen = (name, alt) => screenImage(lang === 'en' ? `${name}-en` : name, alt);
 ...
 return `${head(lang,t.title,t.description,home)}<link rel="alternate" hreflang="ja" href="${origin}/"><link rel="alternate" hreflang="en" href="${origin}/en/"><link rel="alternate" hreflang="x-default" href="${origin}/en/"><script src="/hero.js" defer></script></head><body>
<a class="skip" href="#main">${t.skip}</a>
<header class="site-header"><nav aria-label="${lang === 'ja' ? 'メインナビゲーション' : 'Main navigation'}"><a class="brand" href="${home}" aria-label="${lang === 'ja' ? 'Izzy ホーム' : 'Izzy home'}">${logo}</a><div class="nav-links"><a href="#features">${t.nav[0]}</a><a href="#faq">${t.nav[1]}</a></div><div class="nav-actions"><a class="language" href="${t.other}" lang="${lang === 'ja' ? 'en' : 'ja'}" hreflang="${lang === 'ja' ? 'en' : 'ja'}">${lang === 'ja' ? 'EN' : 'JA'}<span class="sr-only"> · ${t.language}</span></a>${downloadURL ? `<a class="nav-cta" href="${esc(downloadURL)}">${apple}${t.download}</a>` : ''}</div></nav></header>
<main id="main">
<section class="hero" aria-labelledby="hero-title"><div class="hero-copy"><h1 id="hero-title">${t.headline}</h1><p class="hero-intro">${t.intro.replaceAll('\n','<br>')}</p><div class="hero-actions">${action}</div><p class="availability">${status}</p></div>
<div class="hero-media"><div class="hero-video"><video src="/assets/hero-${lang}.mp4" poster="/assets/hero-${lang}.webp" width="720" height="1566" autoplay muted loop playsinline preload="auto" aria-label="${esc(t.video)}"></video><button class="video-toggle" type="button" hidden aria-label="${t.pause}" data-play="${t.play}" data-pause="${t.pause}">${pauseIcon}${playIcon}</button></div></div></section>
<section class="features section" id="features" aria-labelledby="features-title"><div class="section-heading"><h2 id="features-title">${t.featureTitle}</h2></div><div class="feature-grid">${t.features.map((f,i)=>`<article class="feature"><div class="feature-image feature-image-${i}">${screen(f.image,f.alt)}</div><h3>${f.title}</h3><p>${f.text}</p></article>`).join('')}</div></section>
...
</main><footer class="site-footer"><div class="footer-brand"><a class="brand" href="${home}">${logo}</a></div><div class="footer-links"><nav aria-label="${t.footerNavigation}"><a href="#features">${t.nav[0]}</a><a href="#faq">${t.nav[1]}</a></nav><nav aria-label="${t.footerInfo}"><a href="/support/">${t.support}</a><a href="/privacy/">${t.privacy}</a><a href="${t.other}" lang="${lang==='ja' ? 'en' : 'ja'}">${t.language}</a></nav></div><div class="footer-bottom"><a href="https://unvalley.me">${t.made} ↗</a><span>© ${new Date().getUTCFullYear()} Izzy</span></div></footer></body></html>`;
}
