...
<div class="hero-screens">${phone('library-current',lang === 'ja' ? 'イディオムを一覧で探せるIzzyのフレーズ画面' : 'Izzy Phrases screen listing idioms','hero-side hero-library')}${phone('today-current',lang === 'ja' ? 'bring upと今日の学習目標を表示するIzzyのToday画面' : 'Izzy Today with bring up and the daily learning goal','hero-main',true)}${phone('idiom-current',t.features[1].alt,'hero-side hero-notes')}</div></section>
