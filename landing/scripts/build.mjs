...
const check = '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="m5 12 4 4L19 6" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function page(lang) {
 ...
 const faq = t.faqs.filter((_, i) => !downloadURL || i < t.faqs.length - 1);
...
<header class="site-header"><nav aria-label="${lang === 'ja' ? 'メインナビゲーション' : 'Main navigation'}"><a class="brand" href="${home}" aria-label="${lang === 'ja' ? 'Izzy ホーム' : 'Izzy home'}"><img src="/assets/wordmark.svg" alt="Izzy" width="90" height="43"></a><div class="nav-links"><a href="#features">${t.nav[0]}</a><a href="#plans">${t.nav[1]}</a><a href="#faq">${t.nav[2]}</a></div><div class="nav-actions"><a class="language" href="${t.other}" lang="${lang === 'ja' ? 'en' : 'ja'}" hreflang="${lang === 'ja' ? 'en' : 'ja'}">${lang === 'ja' ? 'EN' : 'JA'}<span class="sr-only"> · ${t.language}</span></a>${downloadURL ? `<a class="nav-cta" href="${esc(downloadURL)}">${apple}${t.download}</a>` : ''}</div></nav></header>
<main id="main">
...
<section class="plans section" id="plans" aria-labelledby="plans-title"><div class="section-heading"><h2 id="plans-title">${t.plansTitle}</h2></div><div class="plan-grid"><article class="plan"><h3>${t.free}</h3><p class="plan-amount">${t.freeTag}</p><ul>${t.freeItems.map(s=>`<li>${check}${s}</li>`).join('')}</ul></article><article class="plan pro"><h3>Izzy Pro</h3><p class="plan-amount">${t.proTag}</p><p class="plan-price-note">${t.proPriceNote}</p><ul>${t.proItems.map(s=>`<li>${check}${s}</li>`).join('')}</ul></article></div><p class="plan-note">${t.planNote}</p><ul class="trust-notes">${t.gentleItems.map(s=>`<li>${check}${s}</li>`).join('')}</ul></section>
<section class="faq section" id="faq" aria-labelledby="faq-title"><h2 id="faq-title">${t.faqTitle}</h2><div class="faq-list">${faq.map(([q,a])=>`<details><summary>${q}<span class="summary-icon" aria-hidden="true">+</span></summary><p>${a}</p></details>`).join('')}</div></section>
<section class="closing"><h2>${t.endTitle}</h2><div class="hero-actions">${action}</div><p class="availability">${status}</p></section>
</main><footer class="site-footer"><div class="footer-brand"><a class="brand" href="${home}"><img src="/assets/wordmark.svg" alt="Izzy" width="90" height="43"></a></div><div class="footer-links"><nav aria-label="${t.footerNavigation}"><a href="#features">${t.nav[0]}</a><a href="#plans">${t.nav[1]}</a><a href="#faq">${t.nav[2]}</a></nav><nav aria-label="${t.footerInfo}"><a href="/support/">${t.support}</a><a href="/privacy/">${t.privacy}</a><a href="${t.other}" lang="${lang==='ja' ? 'en' : 'ja'}">${t.language}</a></nav></div><div class="footer-bottom"><a href="https://unvalley.me">${t.made} ↗</a><span>© ${new Date().getUTCFullYear()} Izzy</span></div></footer></body></html>`;
}
