...
// The count comes from the shipped catalog, so an expansion cannot leave the store copy behind.
...
for(const source of sources){
 ...
 const header=`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}"><rect width="${w}" height="${h}" fill="#FAFAF9"/><g transform="translate(${left} 40) scale(${pad?0.075:0.062})">${mark}</g><g font-family="Helvetica,Arial,Hiragino Sans,sans-serif" fill="#202020" font-size="${font}" letter-spacing="-3">${c.title.map((t,i)=>`<text x="${left}" y="${245+i*116}">${escape(t)}</text>`).join('')}</g><text x="${left}" y="455" font-family="Helvetica,Arial,Hiragino Sans,sans-serif" font-size="${pad?34:30}" fill="#636764">${escape(c.detail)}</text></svg>`;
 ...
 manifest.push({file,width:w,height:h,hasAlpha:false,sha256:sha(output),sourceCapture:source.file,sourceSHA256:source.sha256,resultBundle:source.resultBundle,test:source.test,access:c.access,exampleData:c.exampleData??false,catalogCount,catalogSHA256:source.catalogSHA256});
}
