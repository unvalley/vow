#!/bin/bash
set -euo pipefail
if [[ $# -ne 1 || -z "${IZZY_TEAM_ID:-}" ]]; then
  echo 'Usage: IZZY_TEAM_ID=... scripts/export_app_store.sh /path/to/signed.xcarchive' >&2; exit 2
fi
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
python3 "$ROOT/scripts/validate_archive.py" "$1" signed
OPTIONS="$(mktemp -t izzy-export).plist"
trap 'rm -f "$OPTIONS"' EXIT
python3 - "$OPTIONS" "$IZZY_TEAM_ID" <<'PY'
import plistlib,sys
